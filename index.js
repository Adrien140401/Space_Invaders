...
function detectKey(e){
    squares[tireurIndex].classList.remove('tireur')
    if (e.key == 'q' && tireurIndex >= width && tireurIndex >= 141 && tireurIndex != 200 && tireurIndex != 180 && tireurIndex != 160) {
        
        tireurIndex -= 1 //si conditions valide tireur index -1
        console.log(e.key)
    }
    else if(e.key == 'd' && tireurIndex <= 218 && width && tireurIndex != 199 && tireurIndex != 179 && tireurIndex != 159){
         
        tireurIndex += 1 //si conditions valide tireur index +1
        console.log(e.key)
    }
    else if(e.key == 'z' && tireurIndex >= 160 && width){
        tireurIndex -=20
        console.log(e.key)
    }
    else if(e.key == 's' && tireurIndex <= 199){
        tireurIndex +=20
        console.log(e.key)
    }
    squares[tireurIndex].classList.add('tireur')
...
function add() {
    tick();
        chrono.textContent = "Timer : " +(min > 9 ? min : "0" + min)
                + ":" + (sec > 9 ? sec : "0" + sec);
    timer();
}
function timer() {
